...
## V3.3 信任轉換層 + 平板優化（2026/05/14 夜班）

### 「已驗證」徽章系統
- 上傳了真實對帳單的案例 → 自動加綠色「✓ 已驗證」徽章（脈動動畫）
- **首頁案例卡片** + **案例列表卡片** + **表格列** 三處同步
- 客戶一眼看出哪些有鐵證

### 首頁新增「已驗證對帳單」統計卡
- Hero 從 4 欄統計擴展為 5 欄
- 第 4 欄綠色顯示「N 件 ・ 已驗證對帳單」
- 數字自動從 manifest 計算 + count-up 動畫

### 案例頁上一案/下一案 真實導航
- 取代原本空泛的「← 返回 / → 查看其他」
- 改成「← 上一案 ・ U0033 / 2,500 萬」「→ 下一案 ・ K3007 / 1,100 萬」
- 依金額大→小排序，USD 換算 1K = 32 萬 NT
- 全部 69 案頁正確銜接

### 每案專屬 OG 分享圖
- 每案 HTML 加入完整 OG meta（title / description / image / url）
- **JS 動態升級**：如果該案有對帳單（manifest 有記錄），OG image 改指向第 1 張對帳單
- 客戶在 LINE / FB 分享案例連結時，預覽圖就是真實對帳單，視覺衝擊強

### PWA — 可加到手機桌面
- 新增 `manifest.webmanifest`
- 圖示用 SVG 內嵌（DL 字樣 + 金色三角，無需額外圖檔）
- Apple Touch Icon、Mobile Web App 支援
- 客戶按手機瀏覽器「加到主畫面」就能像 App 一樣使用

### 完整響應式（iPad / 手機橫直式）
| 斷點 | 處理 |
|---|---|
| `< 480` | 手機直式：Hero 1 欄、字級縮小、案例 1-2 欄 |
| `481-720` | 手機橫式：Hero 1 欄、案例 2 欄 |
| `721-1024` | 平板直式：Hero 1 欄、案例 2-3 欄、商品 2 欄 |
| `1025-1366` | 平板橫式：桌機版面、Hero 縮一點 |
| `> 1366` | 桌機：完整版面 |

額外手機優化：
- 觸控點擊熱區（active state 取代 hover）
- Lightbox **左右滑動切換**對帳單
- 對帳單卡片邊框、字級、間距分尺寸調整

### 列印增強
- 強制深色文字（取消金色漸層）
- 對帳單 page-break-inside: avoid
- 隱藏所有浮動按鈕、CTA、進度條
- @page 邊距 12mm
